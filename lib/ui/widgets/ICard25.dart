...
class _ICard25State extends State<ICard25> {
  var _titleStyle = const TextStyle(fontSize: 16);
  ...
  _getVerticalText() {
    ...
    if (_actionType == 1) {
      ...
      if (widget.data2?.isNotEmpty ?? false) {
        var temp = <double>[];
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.titleStyle != null) _titleStyle = widget.titleStyle!;
    if (widget.bottomTextStyle != null) {
      _bottomTextStyle = widget.bottomTextStyle!;
    }
    if (widget.action != null) _actionStyle = widget.action!;

    List<Point> _x = [];

    Widget _bottomTexts = Container();
    ...
    if (_dateByType != null) {
      ...
      for (var dataItem in _dateByType) {
        if (dataItem > max) max = dataItem;
      }
      var min = max;
      for (var dataItem in _dateByType) {
        if (dataItem == 0 && _startNull) continue;
        if (dataItem < min) min = dataItem;
        _startNull = false;
      }
        ...
        children: <Widget>[
          ...
          Container(
            margin: EdgeInsets.only(
                left: 5, top: widget.height * 0.2, bottom: widget.height * 0.2),
            child: Column(
          ...
          Container(
              ...
              margin: EdgeInsets.only(
                  left: 20, right: 20, top: widget.height * 0.05),
              child: Row(
          ...
          Container(
            ...
            child: CustomPaint(
              painter: Painter(
                  _x, widget.colorLine, widget.shadowColor, widget.data!),
              size: Size.infinite,
            ),
          ),
        ],
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var point in _x) {
      if (first) {
        point0X = point.x;
        point0Y = point.y;
        path.moveTo(point0X - 10, point0Y as double);
        path.lineTo(point0X as double, point0Y);
        point1X = point.x;
        point1Y = point.y;
        startX = point1X;
        startY = point1Y;
      ...
      path.quadraticBezierTo(cx, point0Y as double, x, y);
      cx = startX + (point1X - point0X) * 0.60;
      path.quadraticBezierTo(cx, point1Y as double, point1X as double, point1Y);
      startX = point1X;
      startY = point1Y;
    }
    path.lineTo(point1X + 10, point1Y as double);
    ...
    for (var point in _x) {
      canvas.drawLine(Offset(point.x as double, point.y as double),
          Offset(point.x as double, point.y as double), paintDotsBack);
    }

    for (var point in _x) {
      canvas.drawLine(Offset(point.x as double, point.y as double),
          Offset(point.x as double, point.y as double), paintDotsWhite);
    }
  }
